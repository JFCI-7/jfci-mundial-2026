...
- Flujo de sync en `db.js`:
  ...
  - `pushMetadataNow()` → escribe `m:<emailid>={has_pin:true}` de inmediato (best-effort). Se llama justo después de `setUserCredentials` y también en `pullFromServer` cuando se detecta un nuevo user con PIN (404 path). Garantiza que la metadata existe desde el primer login, no esperando a la primera predicción. Sin esto, un usuario con PIN que hace logout sin predicciones y vuelve a intentar con PIN incorrecto no podría ser detectado.
